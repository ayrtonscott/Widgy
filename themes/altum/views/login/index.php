        ...
        <div class="col-xs-12 col-md-12 col-lg-10">
            ...
            <div class="card border-0 flex-row">
                <div class="card-body shadow-md p-5">
                    ...
                    <form action="" method="post" class="mt-4" role="form">
                        <div class="form-group">
                            ...
                            <input id="email" type="text" name="email" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('email') ? 'is-invalid' : null ?>" placeholder="<?= l('login.form.email_placeholder') ?>" value="<?= $data->values['email'] ?>" required="required" autofocus="autofocus" />
                            <?= \Altum\Alerts::output_field_error('email') ?>
                        </div>

                        <div class="form-group">
                            <label for="password"><?= l('login.form.password') ?></label>
                            <input id="password" type="password" name="password" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('password') ? 'is-invalid' : null ?>" placeholder="<?= l('login.form.password_placeholder') ?>" value="<?= $data->user ? $data->values['password'] : null ?>" required="required" />
                        ...
                        <?php if($data->user && $data->user->twofa_secret && $data->user->status == 1): ?>
                            <div class="form-group">
                                ...
                                <input id="twofa_token" type="text" name="twofa_token" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('twofa_token') ? 'is-invalid' : null ?>" placeholder="<?= l('login.form.twofa_token_placeholder') ?>" required="required" autocomplete="off" />
                ...
                <div class="card-image card-image-login shadow-md p-5">
                    ...
                    <p class="h4 text-white"><?= l('login.display.total_track_notifications') ?></p>
                </div>
            </div>
