...
<section class="container pt-5">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between mb-5">
        <div>
            <h2 class="h3"><?= l('account_plan.header') ?></h2>
        </div>

        <?php if(settings()->payment->is_enabled): ?>
            <div class="col-auto p-0">
                <?php if($this->user->plan_id == 'free'): ?>
                    <a href="<?= url('plan/upgrade') ?>" class="btn btn-primary"><i class="fa fa-fw fa-arrow-up"></i> <?= l('account.plan.upgrade_plan') ?></a>
                <?php else: ?>
                    <a href="<?= url('plan/renew') ?>" class="btn btn-primary"><i class="fa fa-fw fa-sync-alt"></i> <?= l('account.plan.renew_plan') ?></a>
                <?php endif ?>
            </div>
        <?php endif ?>
    </div>
    ...
    <?php if($this->user->plan_id != 'free' && $this->user->payment_subscription_id): ?>
        <div class="mt-8 d-flex justify-content-between">
            <div>
                <h2 class="h3"><?= l('account_plan.cancel.header') ?></h2>
                <p class="text-muted"><?= l('account_plan.cancel.subheader') ?></p>
            </div>

            <div class="col-auto">
                <a href="<?= url('account-plan/cancel_subscription' . \Altum\Middlewares\Csrf::get_url_query()) ?>" class="btn btn-secondary" onclick='return confirm(<?= json_encode(l('account_plan.cancel.confirm_message')) ?>)'><?= l('account_plan.cancel.cancel') ?></a>
            </div>
        </div>
    <?php endif ?>
