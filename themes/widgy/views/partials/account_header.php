...
<div class="d-flex mb-5 mb-lg-0">
    ...
    <div class="d-flex flex-column">
        ...
        <div>
            <a href="<?= url('account-plan') ?>" class="badge badge-success"><?= sprintf(l('account.plan.header'), $this->user->plan->name) ?></a>

            <?php if($this->user->plan_id != 'free' && (new \DateTime($this->user->plan_expiration_date)) < (new \DateTime())->modify('+5 years')): ?>
                <small><?= sprintf(l('account.plan.subheader'), '<strong>' . \Altum\Date::get($this->user->plan_expiration_date, 2) . '</strong>') ?></small>
            <?php endif ?>
        </div>
    </div>
</div>
