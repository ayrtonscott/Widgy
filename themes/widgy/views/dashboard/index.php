...
<header class="header">
    <div class="container">
        ...
        <div class="d-flex justify-content-between">
            <h1 class="h2"><span class="underline"><?= l('dashboard.header') ?></span></h1>
        </div>
        ...
        <p>
            ...
            <?php if(settings()->payment->is_enabled): ?>
                <span>(<a href="<?= url('plan/upgrade') ?>"><?= l('account.plan.renew') ?></a>)</span>
            <?php endif ?>
        </p>
        ...
        <?php if($this->user->plan_settings->notifications_impressions_limit != -1): ?>
            ...
            <p class="text-muted">
                <?=
                    sprintf(l('account.plan.notifications_impressions_limit'),
...
<section class="container">
            ...
            <div>
                <?php if($this->user->plan_settings->campaigns_limit != -1 && $data->campaigns_total >= $this->user->plan_settings->campaigns_limit): ?>
                    <button type="button" data-toggle="tooltip" title="<?= l('campaign.error_message.campaigns_limit') ?>" class="btn btn-primary disabled">
                        <i class="fa fa-fw fa-sm fa-plus"></i> <?= l('dashboard.campaigns.create') ?>
                    </button>
                <?php else: ?>
                    <button type="button" data-toggle="modal" data-target="#create_campaign" class="btn btn-primary"><i class="fa fa-fw fa-sm fa-plus"></i> <?= l('dashboard.campaigns.create') ?></button>
                <?php endif ?>
            </div>
                        ...
                        <form action="" method="get" role="form">
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                    <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('dashboard.filters.search_by_name') ?></option>
                                    <option value="domain" <?= $data->filters->search_by == 'domain' ? 'selected="selected"' : null ?>><?= l('dashboard.filters.search_by_domain') ?></option>
                                </select>
                            </div>
                            ...
                            <div class="form-group px-4">
                                ...
                                <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                    ...
                                    <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('dashboard.filters.order_by_name') ?></option>
                                </select>
                            </div>
    ...
    <?php if(count($data->campaigns)): ?>
        <div class="table-responsive table-custom-container mt-3">
            <table class="table table-custom">
                <thead>
                <tr>
                    <th><?= l('dashboard.campaigns.name') ?></th>
                    <th class="d-none d-md-table-cell"><?= l('dashboard.campaigns.date') ?></th>
                    <th><?= l('dashboard.campaigns.is_enabled') ?></th>
                    <th></th>
                </tr>
                </thead>
                <tbody>
                ...
                <?php foreach($data->campaigns as $row): ?>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex">
                                <div class="custom-control custom-switch" data-toggle="tooltip" title="<?= l('dashboard.campaigns.is_enabled_tooltip') ?>">
                                    <input
                                            type="checkbox"
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    <a href="<?= url('campaign/' . $row->campaign_id) ?>" class="dropdown-item"><i class="fa fa-fw fa-sm fa-server mr-1"></i> <?= l('global.view') ?></a>
                                    <a href="<?= url('campaign/' . $row->campaign_id . '/statistics') ?>" class="dropdown-item"><i class="fa fa-fw fa-sm fa-chart-bar mr-1"></i> <?= l('campaign.statistics.link') ?></a>
                                    <a href="#" data-toggle="modal" data-target="#update_campaign" data-campaign-id="<?= $row->campaign_id ?>" data-name="<?= $row->name ?>" data-domain="<?= $row->domain ?>" data-include-subdomains="<?= (bool) $row->include_subdomains ?>" class="dropdown-item"><i class="fa fa-fw fa-sm fa-pencil-alt mr-1"></i> <?= l('global.edit') ?></a>
                                    ...
                                    <a
                                        ...
                                        class="dropdown-item"
                                    ><i class="fa fa-fw fa-sm fa-code mr-1"></i> <?= l('campaign.header.pixel_key') ?></a>

                                    <?php if($this->user->plan_settings->custom_branding): ?>
                                        <a href="#" data-toggle="modal" data-target="#custom_branding_campaign" data-campaign-id="<?= $row->campaign_id ?>" data-branding-name="<?= $row->branding->name ?? '' ?>" data-branding-url="<?= $row->branding->url ?? '' ?>" class="dropdown-item"><i class="fa fa-fw fa-sm fa-random mr-1"></i> <?= l('campaign.header.custom_branding') ?></a>
                                    <?php endif ?>

                                    <a href="#" data-toggle="modal" data-target="#campaign_delete_modal" data-campaign-id="<?= $row->campaign_id ?>" class="dropdown-item"><i class="fa fa-fw fa-sm fa-times mr-1"></i> <?= l('global.delete') ?></a>
                                </div>
                            </div>
                            </div>
                        </td>
    ...
    <?php else: ?>
        ...
        <div class="d-flex flex-column align-items-center justify-content-center">
            ...
            <p><?= l('dashboard.campaigns.no_data') ?></a></p>
        </div>
